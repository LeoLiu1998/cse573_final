...
class Episode:
    ...
    def __init__(self, args, gpu_id, rank, strict_done=False):
        ...
        self.actions_list = [{'action': a} for a in BASIC_ACTIONS + [DONE_TOMATO, DONE_BOWL]]
        self.actions_taken = []
    ...
    def judge(self, action):
        ...
        if self.args.improve:
            ...
            for i in range(len(self.distances)):
                ...
                if target in visible_objects:
                    ...
                    if distance2agent < self.distances[i] and self.done_each_action[i] != 1:
        ...
        if action['action'] in [DONE_TOMATO, DONE_BOWL]:
            ...
            if not self.args.many_dones:
                if self.done_each_action[done_id] != 1:
                    self.done_each_action[done_id] = 1
